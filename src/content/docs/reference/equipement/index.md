...
**Dégâts finaux = Code** *(de l'arme/effet)* **+ max(Marge, [Dé rouge](resolution.md)) — PA** *(min 0).*
...
- **Dé rouge** : [dé d'intensité](resolution.md) — explosif si indiqué.
...
À bas niveau de [compétence](competences.md), le dé rouge peut compenser une faible marge. À haut niveau, la compétence domine naturellement. Le dé rouge explosif (déclenché par l'[avantage](resolution.md) ou un critique) reste le levier des personnages puissants — il peut toujours surprendre, mais ne s'empile plus avec une déjà grande marge.
</div>
...
seuil de réussite : la [marge de réussite](resolution.md), c'est le nombre de balles qui
touchent (maximum 3)
